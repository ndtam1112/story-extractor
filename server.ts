...
${text}`;
  ...
  app.all('/api/extract', async (req, res) => {
    try {
      ...
      // Optional API key protection (set API_KEY in environment variables)
  ...
  // Frontend API endpoint to fetch and extract story content
  ...
  app.post('/api/export-to-sheets', async (req, res) => {
    try {
      ...
      // 4. Format the sheet beautifully
      await sheets.spreadsheets.batchUpdate({
        spreadsheetId: spreadsheetId!,
        requestBody: {
          requests: [
            // Format Story Title (Bold, larger font, wrapped)
            {
              repeatCell: {
                range: {
                  sheetId: 0,
            ...
            // Format Chapter Title (Bold, medium font)
            {
              repeatCell: {
                range: {
                  sheetId: 0,
            ...
            // Format Paragraphs (Wrapped text, nice padding)
            {
              repeatCell: {
                range: {
                  sheetId: 0,
            ...
            // Resize Column A to be wide and readable
            {
